...
const tableColumn = [{
    ...
    Header: "Total Views", accessor: "totalViews",
}, {
...
const ArticleList = () => {
    ...
    const fetchCategoryList = async () => {
        setLoader(true)
        let resultSet = [];
        await getMethod(categoryListUrl).then((response) => {
            let sl = 1;
            for (let item of response.data.data) {
                resultSet.push({
                    sl: sl++, label: item.title, value: item.id
                })
            }
    ...
    return (<>
        ...
        <ArticleForm
            modalShow={modal}
            selectedArticle={Object.keys(selectedArticle).length > 0 ? selectedArticle : undefined}
